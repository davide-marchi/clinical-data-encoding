...
def trainIMEO(  
    ...
    for i in range(num_epochs):
        ...
        for metric in metrics:
            if metric == 'tr_loss':
                metrics_hystory[metric].append(model.training_step(train_data).item())
            elif metric == 'val_loss':
                metrics_hystory[metric].append(model.training_step(val_data).item())
            elif metric == 'val_r2':
                metrics_hystory[metric].append(model.compute_r_squared(val_data, binary_clumns))
            elif metric == 'val_acc':
                metrics_hystory[metric].append(model.compute_accuracy(val_data, binary_clumns))
            elif metric == 'val_a2':
                metrics_hystory[metric].append(model.weighted_measure(val_data, binary_clumns))
        if (print_every != 0):
